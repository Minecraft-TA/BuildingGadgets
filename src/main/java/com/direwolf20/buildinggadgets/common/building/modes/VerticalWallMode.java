...
public class VerticalWallMode extends AtopSupportedMode {
    ...
    public IPlacementSequence computeWithTransformed(EntityPlayer player, BlockPos transformed, BlockPos original, EnumFacing sideHit, ItemStack tool) {
        ...
        if (EnumFacingPortUtil.getAxis(sideHit).isVertical())
            return Wall.extendingFrom(transformed.offset(EnumFacingPortUtil.getOpposite(sideHit)), sideHit, EnumFacingPortUtil.getHorizontalFacing(player), radius, MathTool.isEven(range) ? 1 : 0);
