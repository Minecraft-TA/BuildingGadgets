...
public class InventoryManipulation {
    ...
    private static ItemStack insertIntoInventory(IInventory inventory, ItemStack target, InventoryType type) {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); i++) {
            ...
            if (containerItem != null && containerItem.getItem() == target.getItem() && containerItem.getItemDamage() == target.getItemDamage()) {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); i++) {
            ...
            ItemStack containerItem = inventory.getStackInSlot(i);
            if (containerItem != null || !inventory.isItemValidForSlot(i, target) || type == InventoryType.PLAYER && i == 40)
                continue;

            ItemStack insertStack = target.copy();
            insertStack.stackSize = Math.min(target.stackSize, Math.min(target.getMaxStackSize(), Math.min(containerItem.getMaxStackSize(), inventory.getInventoryStackLimit())));

            // TODO: Simulation doesn't exist but we should be fine
//            ItemStack stack = inventory.insertItem(i, insertStack, true);
//            if (stack.stackSize == insertStack.stackSize)
//                continue;

            inventory.setInventorySlotContents(i, insertStack);
            target.stackSize -= insertStack.stackSize;
        }
    ...
    public static boolean useItem(ItemStack target, EntityPlayer player, int amountRequired, World world) {
        ...
        for (Pair<InventoryType, IInventory> inv : collectInventories(GadgetGeneric.getGadget(player), player, world, NetworkIO.Operation.EXTRACT)) {
    ...
    private static int extractFromInventory(IInventory inventory, ItemStack target, int amountRequired, EntityPlayer player) {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); i++) {
            ...
            if (containerItem.getItem() == target.getItem() && containerItem.getItemDamage() == target.getItemDamage()) {
                ...
                int amountToExtract = Math.min(stack.stackSize, amountRequired - amountSaturated);
                inventory.decrStackSize(i, amountToExtract);

                amountSaturated += amountToExtract;
            }
    ...
    private static List<Pair<InventoryType, IInventory>> collectInventories(ItemStack gadget, EntityPlayer player, World world, NetworkIO.Operation operation) {
        ...
        // TODO: Do items with inventories exist in 1.7?
    ...
    public static int countItem(ItemStack itemStack, EntityPlayer player, IRemoteInventoryProvider remoteInventory) {
        ...
        if (slots.isEmpty() && invContainers.isEmpty() && count == 0) {
            return 0;
        }
        ...
        for (int slot : slots) {
            ...
            count += stackInSlot == null ? 0 : stackInSlot.stackSize;
        }
    ...
    public static boolean usePaste(EntityPlayer player, int count) {
        ...
        if (slots.size() > 0) {
            for (int slot : slots) {
                ...
                if (pasteStack != null && pasteStack.stackSize >= count) {
    ...
    public static int countInContainer(IInventory container, Item item, int meta) {
        ...
        for (int i = 0; i < container.getSizeInventory(); i++) {
            ...
            if (tempItem != null && tempItem.getItem() == item && tempItem.getItemDamage() == meta) {
    ...
    private static List<Integer> findItemClass(Class<?> c, IInventory itemHandler) {
        ...
        for (int i = 0; i < itemHandler.getSizeInventory(); i++) {
            ItemStack stack = itemHandler.getStackInSlot(i);
            if (stack != null && c.isInstance(stack.getItem())) {
                slots.add(i);
            }
        }
        return slots;
    ...
    public static ItemStack getSilkTouchDrop(IBlockState state) {
        ...
        if (item.getHasSubtypes()) {
            i = state.getBlock().damageDropped(state.getMeta());
        }
    ...
    public static IBlockState getSpecificStates(IBlockState originalState, World world, EntityPlayer player, BlockPos pos, ItemStack tool) {
        ...
        ItemStack item;
        try {
            item = block.getPickBlock(null, world, pos.getX(), pos.getY(), pos.getZ(), player);
        ...
        try {
            placeState = originalState.withMeta(originalState.getBlock().onBlockPlaced(world, pos.getX(), pos.getY(), pos.getZ(), EnumFacing.UP.ordinal(), 0, 0, 0, meta));
        } catch (Exception var8) {
            placeState = originalState.getDefaultState();
        }
        // TODO: Doesn't exist
     ...
     * choose his primary hand first. If neither hands have the desired item stack, it will return null.
     *
     * @return null when neither hands met the parameter.
     */
    public static ItemStack getStackInEitherHand(EntityPlayer player, Class<?> itemClass) {
        ItemStack mainHand = player.getHeldItem();
        if (mainHand != null && itemClass.isInstance(mainHand.getItem()))
            return mainHand;
